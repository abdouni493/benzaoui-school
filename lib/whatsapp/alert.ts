 ...
 *  seul endroit où la clé OpenWA est lue. */

import { isSendablePhone } from "./phone";
import { getTemplate, type MessageLanguage, type WhatsAppTemplateId } from "./templates";
...
/** Construit la charge utile WhatsApp (numéro + nom + texte) d'une alerte de
...
export function buildBalanceAlert(params: {
  ...
  const target = isSendablePhone(parent?.phone)
    ? { phone: parent!.phone!, name: `${parent!.firstName} ${parent!.lastName}`, audience: "parent" as const }
    : isSendablePhone(student.phone)
      ? { phone: student.phone!, name: `${student.firstName} ${student.lastName}`, audience: "student" as const }
      : null;
  ...
  const text = getTemplate(templateId).build(
    {
