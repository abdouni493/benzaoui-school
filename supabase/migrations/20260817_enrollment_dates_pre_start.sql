-- =============================================================================
-- Dates d'inscription / de début d'abonnement + scans AVANT la date de début
-- Run once against the live project (Supabase Dashboard -> SQL Editor).
--
-- Ce que fait cette migration :
--   1. student_subscriptions.subscribed_at — la DATE D'INSCRIPTION (le jour où
--      la réception a inscrit l'élève au module), distincte de start_date.
--      start_date devient la DATE DE DÉBUT de la facturation, pour TOUS les
--      abonnements (cours ET formations), et non plus les seules formations.
--   2. attendance.pre_start — la présence a été enregistrée AVANT la date de
--      début de l'abonnement : la séance est offerte, rien n'est débité, et le
--      prix non facturé est journalisé dans attendance.waived_amount.
--   3. scan_card réécrit :
--        · un abonnement dont la date de début est dans le futur n'est PLUS
--          ignoré (la carte n'est plus refusée « aucune séance aujourd'hui ») ;
--        · avant la date de début, coût = 0, SOLDE STRICTEMENT INCHANGÉ,
--          présence enregistrée et montant offert mémorisé ;
--        · seule la date d'expiration continue d'invalider une inscription.
--   4. mark_attendance : exactement la même règle en saisie manuelle depuis
--      l'écran Présences (feuille du jour / d'un jour passé).
--
--   process_weekly_absences n'est pas modifié : il ancre déjà chaque semaine
--   sur ss.start_date, donc aucune absence n'est facturée avant le début.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Date d'inscription sur l'inscription élève <-> module
-- ---------------------------------------------------------------------------
alter table public.student_subscriptions
  add column if not exists subscribed_at date;

comment on column public.student_subscriptions.subscribed_at is
  'Date d''inscription : le jour où l''élève a été inscrit à ce module.';
comment on column public.student_subscriptions.start_date is
  'Date de début de facturation : avant cette date, les séances sont offertes (aucun débit).';

-- Reprise de l'existant : les inscriptions déjà datées gardent leur date de
-- début comme date d'inscription (meilleure approximation disponible).
update public.student_subscriptions
   set subscribed_at = start_date
 where subscribed_at is null
   and start_date is not null;

-- ---------------------------------------------------------------------------
-- 2. La trace « séance offerte car avant le début » sur la présence
-- ---------------------------------------------------------------------------
-- pre_start = true  -> waived_amount contient le prix qui n'a PAS été débité.
alter table public.attendance
  add column if not exists pre_start boolean not null default false;

create index if not exists attendance_pre_start_idx
  on public.attendance (pre_start) where pre_start;

-- ---------------------------------------------------------------------------
-- 3. scan_card — avant la date de début : présence oui, débit non
-- ---------------------------------------------------------------------------
-- Identique à la version précédente (fenêtre horaire, scan inter-groupes,
-- anti-double-badge, réductions, périodes gratuites), avec DEUX changements :
--   a) une inscription dont start_date est postérieure à aujourd'hui reste
--      éligible (elle n'est plus filtrée) — l'élève peut donc badger ;
--   b) dans ce cas le coût passe à 0, le solde n'est pas touché, aucune ligne
--      de compte n'est écrite et le prix normal part dans waived_amount.
...
begin
  ...
  -- La date de début n'entre PAS dans l'éligibilité : elle ne décide que du
  -- débit (voir plus bas). Seule l'expiration invalide une inscription.
  ...
  -- Celle de son propre groupe en priorité ; sinon celle de l'inscription
  -- (même classe + même module) qui a déjà commencé, s'il y en a une.
  ...
  if v_is_free_period or v_before_start then
    -- Période gratuite OU abonnement pas encore commencé : rien n'est débité.
  ...
  -- Part enseignant : sur une séance offerte (période gratuite quand elle le
  -- prévoit, ou séance antérieure au début de l'abonnement), elle suit le prix
  -- normal — l'enseignant a bien assuré le cours.
  v_teacher_base := case
  ...
  begin
    ...
    values (v_student.id, v_matched.id, p_when, v_cost, v_status, not v_own_group,
       ...
       v_before_start and not v_is_free_period);
  ...
  return jsonb_build_object(
    ...
    'preStart', v_before_start and not v_is_free_period,
...
-- ---------------------------------------------------------------------------
-- 4. mark_attendance — même règle en saisie manuelle
-- ---------------------------------------------------------------------------
-- La feuille de présence (du jour ou d'un jour passé) applique exactement la
-- même règle que le badge : avant la date de début de l'abonnement, la présence
-- est enregistrée et RIEN n'est débité.
...
-- ---------------------------------------------------------------------------
-- 5. Grants (signatures inchangées, ré-affirmés par sécurité)
-- ---------------------------------------------------------------------------
